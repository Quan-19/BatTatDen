...
const TurnOnOffLedScreen_Mqtt = () => {
  ...
  const publishTopic = (deviceStatus) => {
    const s =
      '{"message":"turn on/off led","name":"led","status":"' +
      deviceStatus +
      '"}';

    var message = new Paho.MQTT.Message(s);
    message.destinationName = "NGUYENQUOCBAO/v1/device/led/rpc";
    client.send(message);
  };

  const subscribeTopic = () => {
    client.subscribe("NGUYENQUOCBAO/v1/device/led/rpc", { qos: 0 });
  };

  const onMessageArrived = async (message) => {
    console.log("onMessageArrived:" + message.payloadString);
    setMsg(message.payloadring);
